...
def download_video(url, cookie_path=None):
    ...
    # [김지연 4.3] 최후의 수단: 헤더 제거 및 순수 모바일 클라이언트 모드
    # 이전 버전에서 PC User-Agent와 Android Client 설정이 충돌했을 가능성 차단
    ydl_opts = {
        'format': 'bestvideo+bestaudio/best',
        'outtmpl': output_template,
        'noplaylist': True,
        
        # 1. 모바일 앱(Android, iOS)으로 완벽 위장
        'extractor_args': {
            'youtube': {
        ...
        # 2. [수정] 인위적인 헤더 조작 제거 (yt-dlp가 알아서 모바일 헤더를 쓰도록 둠)
        # 'http_headers': { ... } <- 이 부분 삭제함
...
st.set_page_config(page_title="AI Shorts Maker Pro (김지연 4.3)", layout="wide")

st.title("🎬 AI 숏폼 자동 생성기 Pro (김지연 4.3)")
st.markdown("Gemini 2.5 Flash | **순수 모바일 클라이언트 모드** | **담당자: 김지연**")
    ...
    if not youtube_url:
        ...
        with st.status("작업 진행 중...", expanded=True) as status:
            ...
            try:
                ...
                # [김지연 4.3] 에러 메시지 수정 (로컬 설치 권장)
                if "403" in str(e):
                    st.warning("🚨 **최후의 수단까지 막혔습니다.**")
                    st.info("""
                    유튜브가 클라우드 서버의 접근을 완전히 차단한 것으로 보입니다.
                    쿠키 파일과 위장술로도 해결되지 않는다면, **로컬(내 컴퓨터) 설치**가 유일한 답입니다.
                    
                    **[로컬 설치 방법]**
                    1. 지금 작업하신 `shorts_maker` 폴더를 팀원들에게 공유하세요.
                    2. 각자의 컴퓨터에서 실행하면 403 에러 없이 바로 잘 될 겁니다!
                    """)
                st.stop()
